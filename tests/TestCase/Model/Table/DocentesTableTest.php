...
class DocentesTableTest extends TestCase
{
    ...
    {
        parent::setUp();
        $this->Docentes = TableRegistry::getTableLocator()->get('Docentes');
    }

    public function testValidationNomeRequired(): void
    {
        $docente = $this->Docentes->newEntity(['nome' => '']);
        $this->assertFalse($this->Docentes->save($docente));
        $this->assertArrayHasKey('nome', $docente->getErrors());
    }

    public function testValidationNomeMaxLength(): void
    {
        $docente = $this->Docentes->newEntity(['nome' => str_repeat('a', 201)]);
    ...
    {
        $docente = $this->Docentes->newEntity(['nome' => 'Teste', 'status' => 'bogus']);
        $errors = $docente->getErrors();
    ...
    {
        $docente = $this->Docentes->newEntity(['nome' => 'Teste', 'email' => 'not-an-email']);
        $errors = $docente->getErrors();
    ...
    {
        $docente = $this->Docentes->newEntity(['nome' => 'Teste', 'status' => 'active']);
        $this->assertSame('ativo', $docente->status);

        $docente = $this->Docentes->newEntity(['nome' => 'Teste', 'status' => 'retired']);
        $this->assertSame('aposentado', $docente->status);

        $docente = $this->Docentes->newEntity(['nome' => 'Teste', 'status' => 'inactive']);
        $this->assertSame('inativo', $docente->status);
    }

    public function testBeforeMarshalKeepsCanonicalStatus(): void
    {
        $docente = $this->Docentes->newEntity(['nome' => 'Teste', 'status' => 'aposentado']);
        $this->assertSame('aposentado', $docente->status);
    }

    public function testBeforeMarshalDropsEmptyStatus(): void
    {
        $docente = $this->Docentes->newEntity(['nome' => 'Teste', 'status' => '']);
        $this->assertNull($docente->status);
    }

    public function testSaveSetsTimestamps(): void
    {
        $docente = $this->Docentes->newEntity(['nome' => 'Novo Docente']);
        $saved = $this->Docentes->save($docente);
    ...
    {
        $docente = $this->Docentes->get(1);
        $this->assertFalse($this->Docentes->delete($docente));
        $this->assertNotNull($this->Docentes->findById(1)->first());
    }

    public function testDeleteCascadesDocenteDisponibilidades(): void
    {
        $docente = $this->Docentes->get(2);
        $this->assertTrue($this->Docentes->delete($docente));

        $this->assertNull($this->Docentes->findById(2)->first());
    ...
    {
        $docente = $this->Docentes->get(3);
        $this->assertTrue($this->Docentes->delete($docente));
        $this->assertNull($this->Docentes->findById(3)->first());
    }
}
